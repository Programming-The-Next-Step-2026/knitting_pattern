...
from knitting_pattern.image_engine import (
    get_hardcoded_heart, scale_pattern_matrix_integer, overlay_pattern_on_grid,
    generate_multipage_pdf_figs, process_uploaded_image
)
...
if st.button("✨ Generate Final Printable Chart", type="primary", use_container_width=True):
    with st.spinner("Generating high-res chart..."):
        ...
        st.session_state.grid = final_chart
...
if "grid" in st.session_state:
    ...
    # 1. THE SLIDER: Let the user control the slicing dynamically!
    st.markdown("### 📄 Print Settings")
    rows_per_page = st.slider(
        "Rows per printed page", 
        min_value=30, max_value=120, value=60, step=10, 
        help="Lower numbers make the grid squares larger and easier to read on paper."
    )
    ...
    # 3. Display every page on the screen sequentially
    for i, fig in enumerate(figs):
        st.pyplot(fig)
    
    # 4. Compile everything into a single multi-page PDF for download
